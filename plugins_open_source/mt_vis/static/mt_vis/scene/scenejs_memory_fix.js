/**
 * This file combines fixes for memory leaks in SceneJS lib: https://redmine.minetronics.vpn/issues/2875
 *
 * @param {type} SceneJS
 * @returns {undefined}
 */
define(['scenejs'], function (SceneJS) {
    /**
     * Fixes clearing publishing reference when node is destroyed
     * @returns {undefined}
     */
    SceneJS.Node.prototype._destroyTree = function () {
        var i, len = this.nodes.length, node;
        this.destroyed = true;
        this._engine.destroyNode(this); // Release node object
        for (i = 0; i < len; i++) {
            node = this.nodes[i];
            //Added missing unpublish for child nodes
            this._engine.scene.unpublish("nodes/" + node.id);
            node._destroyTree();
        }
    };

    /**
     * Hotifxed the pop() method to actually remove cores from the 'cores' stack,
     * letting them to be GC.
     *
     ...
     *
     */
    SceneJS_modelXFormStack = new (function () {
